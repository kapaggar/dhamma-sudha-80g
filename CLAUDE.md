...
- No Drive advanced service and no `DriveApp` — both demand the full `drive` scope. Temp XLS→Sheet conversion goes through the Drive REST API (`driveCreateSheetFromBlob_`/`driveDeleteFile_` in `DanaImport.gs`) under the narrow `drive.file` scope. `UrlFetchApp` is pinned by `urlFetchWhitelist` in `appsscript.json`; new hosts must be added there.
...
| `Tests.gs` | Unit tests |
| `Form.html` / `ImportDialog.html` / `UploadDialog.html` | Donor PAN form / admin import modal / XLS upload fallback modal |
